...
serial_port = serial.Serial('/dev/ttyACM1', 115200, write_timeout=0.1, xonxoff=True)

pose_to_command = {
    'ONE': [28, 0, 0, 0, 0, 0],    # Example array for pose ONE
    'TWO': [28, 28, 0, 0, 0, 0],  # Example array for pose TWO
    'THREE': [28, 28, 28, 0, 0, 0], # Example array for pose THREE
    'FOUR': [28, 28, 28, 28, 0, 0], # Example array for pose FOUR
    'FIVE': [28, 28, 28, 28, 28, 0], # Example array for pose FIVE
    'FIST': [0, 0, 0, 0, 0, 0],   # Example array for pose FIST
    'OK': [28, 0, 28, 0, 0, 0],   # Example array for pose OK
    'PEACE': [28, 28, 0, 0, 28, 0]  # Example array for pose PEACE
}


def send_command_to_robotic_hand(command):
    # Retrieve the array associated with the command from the dictionary
    command_array = pose_to_command.get(command, None)
    
    if command_array:
        # print(f"Sending command to robotic hand: {command_array}")
        try:
            float_bytes = struct.pack('6f', *command_array)
        ...
        packet = bytearray(64)
        packet[0:24] = float_bytes
        
        try:
            serial_port.write(packet)
            print(f"Sent successfully: {command_array}")
...
class HandController:
    ...
    def loop(self):
        while True:
            ...
            self.frame_nb += 1
            for hand in hands:
                if hand.gesture in pose_to_command:  # Corrected this line
                    send_command_to_robotic_hand(hand.gesture)
                    print(f"hands: {hand.gesture}")
